...
class Building(db.Model):
    ...
    building_image = db.Column(db.String(500), default='\
        https://dangerwordfilm.files.wordpress.com/2014/04/coming-soon.png')
    units = db.relationship('Unit', back_populates='building')

    def __init__(
            self, ownerID, name, address,
            description, number_of_units, building_image):
...
class Unit(db.Model):
    ...
    unit_image = db.Column(db.String(500), default='\
        https://dangerwordfilm.files.wordpress.com/2014/04/coming-soon.png')
    building_id = db.Column(db.Integer, db.ForeignKey('\
        building.id'), nullable=False)
    building = db.relationship('Building', back_populates='units')

    def __init__(
            self, description, number_of_bedroom, number_of_living_room,
            number_of_bathroom, vacancy, type_of_rental,
            rent_price, unit_image, building_id):
        self.description = description
